document.addEventListener('DOMContentLoaded', () => {
      ...
      let userData = { nome: '', situacao: '', detalheOutros: '', telefone: '' };
      ...
      situacaoForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const value = situacaoSelect.value;
        if (!value) return alert('Selecione uma opção.');
        if (value === 'Outros' && outrosInput.value.trim() === '') {
          return alert('Descreva sua situação.');
        }
        userData.situacao = value;
        userData.detalheOutros = value === 'Outros' ? outrosInput.value.trim() : '';
      ...
      contatoForm.addEventListener('submit', async (e) => {
        ...
        // ENVIO DESATIVADO – apenas simula
